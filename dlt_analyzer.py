...
def generate_combinations(scores_data: Dict, pattern_data: Dict, arm_rules: pd.DataFrame, weights_config: Dict) -> Tuple[List[Dict], List[str]]:
    ...
    try:
        ...
        from improved_combination_generator import ImprovedCombinationGenerator
        
        # 创建改进的组合生成器
        generator = ImprovedCombinationGenerator(r_scores, b_scores)
        improved_combinations = generator.generate_improved_combinations(num_to_gen)
        
        if improved_combinations:
            # 转换为原有格式
            final_recs = []
            for combo in improved_combinations:
                final_recs.append({
                    'combination': {'red': combo['red'], 'blue': combo['blue']},
                    'score': combo['score'],
                    'strategy': combo.get('strategy', '改进策略')
                })
            
            # 生成输出字符串
            output_strs = [f"改进推荐组合 (Top {len(final_recs)}):"]
            for i, c in enumerate(final_recs):
                ...
                strategy = c.get('strategy', '改进策略')
                output_strs.append(f"  注 {i+1}: 红球 [{r_str}] 蓝球 [{b_str}] (综合分: {c['score']:.2f}) - {strategy}")
            
            return final_recs, output_strs
            
    except ImportError:
        logger.warning("改进组合生成器导入失败，使用原有策略")
    except Exception as e:
        logger.warning(f"改进组合生成失败: {e}，使用原有策略")
    ...
    final_recs = []
    max_common = 2  # 改进：最多2个相同红球，增强多样性
...
if __name__ == "__main__":
    ...
    try:
        ...
        # 单式推荐（显示所有推荐注数）
        simple_content += "📊 单式推荐：\n"
        for rec_line in final_rec_strings[1:]:  # 跳过标题行，显示所有推荐
            if rec_line.strip():
                simple_content += f"{rec_line}\n"
        
        # 复式推荐（与分析报告中的平衡选择逻辑保持一致）
        if final_scores and final_scores.get('red_scores'):
            r_scores = final_scores['red_scores']
            b_scores = final_scores['blue_scores']
            
            # 红球分区选择：确保各区间平衡（与分析报告逻辑一致）
            zone_reds = {1: [], 2: [], 3: []}
            for num, score in sorted(r_scores.items(), key=lambda x: x[1], reverse=True):
                if 1 <= num <= 12:
                    zone_reds[1].append(num)
            ...
            simple_content += f"\n📦 复式参考：\n"
            simple_content += f"红球: {' '.join(f'{n:02d}' for n in complex_red)}\n"
            simple_content += f"蓝球: {' '.join(f'{n:02d}' for n in complex_blue)}\n"
